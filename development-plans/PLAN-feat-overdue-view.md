...
- `test_focused_view_service.py` (modify) — extend `TestGetDayViewTasks` with: task overdue via `must_do_by` alone; task overdue via `target_date` alone; **mixed case — `must_do_by` before `reference_date` and `target_date` after it (and the inverse) — asserting the task is still included, to directly exercise the "earliest of the two wins" criterion the Overview states in prose** (this is the boundary a future edit to the `or_(...)` clause could silently invert without a positional-only-field test catching it); excludes a task whose effective date is today; excludes a task whose effective date is in the future; `overdue=False` (default) preserves the existing exact-match behavior (explicit regression test, not just reliance on pre-existing cases). Existing `TestGetFocusedTasks`/`TestGetDayViewTasks` cases act as regression coverage for the `_query_board_grouped_tasks` refactor itself.
...
3. **[Gap] No test for the mixed-date-fields "earliest wins" case.** Added to the Test plan's `TestGetDayViewTasks` additions (both directions — `must_do_by` earlier and `target_date` earlier).
