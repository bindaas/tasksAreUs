...
**Scope decision (confirmed by user 2026-07-05, see "Open Questions — Resolved" near the end of this file): add inline quick-edit to Focused/Today/Tomorrow only**, mirroring exactly what PR #42 changed on web. All view keeps its existing modal-based edit — that web/mobile divergence predates PR #42 and isn't its job to fix.
...
Open Question #2 (whether this section needed its own Sneezy pass) is now resolved — it has been reviewed. Open Question #1 (narrow extraction vs. duplication) is resolved below, in "Open Questions — Resolved (user decision, 2026-07-05)."

---

## Open Questions — Resolved (user decision, 2026-07-05)

Both prior open questions are now resolved:

1. **Inline quick-edit scope (line 101):** Confirmed — Focused/Today/Tomorrow get inline quick-edit + Links/Complete/Delete to bring them to parity with what All already has, mirroring exactly what web's PR #42 did. All view's existing modal-based edit is unchanged. This was the plan's existing default; no plan changes needed.

2. **Narrow extraction vs. duplication for Links/action-button rendering — superseded by a full shared-component decision.** Investigated web's actual implementation (`frontend/src/components/TaskCardBody.tsx`, `TaskCard.tsx`, `FocusedTaskCard.tsx` — all read in full) rather than relying on the plan's prior assumption: web did **not** do the narrow Links/buttons-only extraction Sneezy suggested for mobile — it fully extracted a single shared `TaskCardBody` component consumed by both `TaskCard.tsx` (All view) and `FocusedTaskCard.tsx` (Focused/Today/Tomorrow), parameterized by `layout: 'inline' | 'stacked'`, `dateDisplay`, `priorityBadge`, `onTogglePriority`, and `renderLabels`. The bottom-vs-top action-button placement is intentional and driven by the `layout` prop (`'stacked'` renders actions in a `justify-end` row below the content; `'inline'` renders them top-right in a flex-between row) — not an unfinished parity gap. This proves the layout divergences Sneezy was concerned about (board-color border strip, badge-as-pill vs. inline star, `isDone` dimming, label-chip spacing) are handleable via props rather than being a reason to avoid a shared component.

   **Decision: mobile does a full shared card-body extraction, mirroring web's architecture**, not the narrower `TaskLinksRow`/`TaskActionButtons`-only split. `TaskRow` (`TasksScreen.tsx:71-154`) and `FocusedTaskCard.tsx` should both consume one new shared component (RN equivalent of `TaskCardBody.tsx`), parameterized analogously (layout/style props, date-display mode, priority-badge style, label rendering as a render-prop or children) rather than duplicating Links/Complete/Delete/title/date/label rendering across the two files. This supersedes the "Not attempting a shared-component extraction on mobile" rationale and its stated correction in the "Folding in PR #45" section above — extraction is now in scope, not skipped.

3. ~~Whether the "Mobile Equivalent of Web PR #42" section needs its own Sneezy pass~~ — resolved 2026-07-04 (second pass); see above.

### `TaskCardBody`'s full prop/rendering surface (addressed per Sneezy's 2026-07-05 review, Gap 3)

The RN port owns Title/Date/Labels/Links/priority-indicator rendering, **plus an Edit button**, alongside Complete/Delete — i.e. its action row renders all of Edit/Complete/Delete together (confirmed by reading `frontend/src/components/TaskCardBody.tsx:103-147` in full), not just Complete/Delete as an earlier draft of this section stated. It requires four callback props: `onEdit`, `onComplete`, `onDelete`, `onTogglePriority` (the last only used when `priorityBadge: 'toggle'`).

- **`onTogglePriority`/`priorityBadge: 'toggle'` has no mobile equivalent and is not needed.** Confirmed by reading both current mobile call sites (`mobile/src/screens/TasksScreen.tsx`'s `TaskRow`, `mobile/src/components/FocusedTaskCard.tsx`): neither has any priority-toggle-from-card affordance today (the ★ badge is read-only in both), and this plan does not add one. Both mobile call sites use `priorityBadge: 'static'`, mirroring web's own `FocusedTaskCard.tsx` (`priorityBadge="static"`, no `onTogglePriority` passed) — only web's `TaskCard.tsx` (All view) uses `'toggle'`, and this plan does not port that behavior to mobile's `TaskRow`.
- **The `task.state === 'pending'` gate on the action row (hiding Edit/Complete/Delete for non-pending tasks) is carried over as-is.** This is expected to be inert in practice once this plan's "Show completed" removal ships, since neither `TaskRow` nor `FocusedTaskCard` should receive a non-pending task in their input at that point — but this is a live behavior change today, since mobile's current `TaskRow` Delete button (`TasksScreen.tsx:143-149`) has no `isDone` gate at all. Implementer should confirm at build time that the "Show completed" removal fully eliminates non-pending tasks from both components' inputs; if any code path still surfaces a non-pending task, this gate will additionally hide its Delete button, a difference from today's ungated behavior.

### Resolving the duplicate-Edit-affordance risk (Sneezy's 2026-07-05 review, Risk 2)

A literal port of `TaskCardBody` gives every consumer a built-in Edit icon. Read both web and mobile's current source to work out how each of `TaskRow` and `FocusedTaskCard.tsx` should wire it, so nothing is duplicated or lost:

- **`FocusedTaskCard.tsx` (stacked layout): drop the separately-planned standalone pencil-icon button from the "Inline quick-edit" section above.** Instead, wire `TaskCardBody`'s own built-in Edit icon to `onEdit={() => setIsEditing(true)}` — this is exactly what web's `FocusedTaskCard.tsx` does (confirmed: `onEdit={() => setIsEditing(true)}`, no separate pencil component anywhere in the file). Mobile's current `FocusedTaskCard.tsx` already has an outer `TouchableOpacity` wrapping the whole card (`onPress={() => onPress(task.id)}`, where `onPress` is passed in from `FocusedView.tsx`/`DayView.tsx` as `onEditPress`, today opening the full `TaskFormScreen` modal for *any* tap on the card). This must gain the same guard web's outer `onClick` has: `onPress={() => { if (!isEditing) onPress(task.id); }}`. Net result, mapping web's "click elsewhere on the card navigates to the full detail page" onto mobile's equivalent concept ("tap elsewhere on the card opens the full edit modal", since mobile has no separate detail page): tapping the dedicated Edit icon opens the lightweight inline `TaskQuickEdit`; tapping anywhere else on a non-editing card still opens the full `TaskFormScreen` modal, unchanged from today; while `isEditing`, the whole-card tap is suppressed so it can't fight with the inline editor's own touchables. No duplicate affordance — the icon and the rest-of-card tap now do two different, deliberate things, same as web.
- **`TaskRow` (inline layout, All view): keep the existing outer `TouchableOpacity`'s `onPress={() => onEditPress(task.id)}` unchanged** (confirmed at `TasksScreen.tsx:85-86` — this opens the `TaskFormScreen` modal today, and there is no dedicated Edit icon anywhere in the current component, only Complete (✓) and Delete (🗑) icons). Wire `TaskCardBody`'s built-in Edit icon to that same `onEditPress(task.id)` callback. This does add one new, previously-absent small Edit icon to the All view's action row (today only Complete/Delete are icons; Edit is whole-row-tap only) — a direct, intentional consequence of the already-approved "mirror web" and "full shared `TaskCardBody`" decisions (web's `TaskCard.tsx` action row has the same three icons), not a new UX decision being made here. Functionally inert change: the icon performs the exact same action (`onEditPress`) the row already performs on tap anywhere else.

### Files to Modify (further addition, resolving Open Question #1 above)

- New: `src/components/TaskCardBody.tsx` (RN port of the web component of the same name) — shared presentational body for both `TaskRow` and `FocusedTaskCard.tsx`, parameterized by layout (`'inline' | 'stacked'`), date-display mode, priority-badge style (`'static'` for both mobile call sites — see above), and a labels render-prop; owns Title/Date/Labels/Links/priority-indicator/Edit/Complete/Delete rendering, gated by `task.state === 'pending'` for the action row (see above)
- `src/screens/TasksScreen.tsx`'s `TaskRow` (lines 71-154) — refactored to consume `TaskCardBody` with `layout: 'inline'`; outer `TouchableOpacity`'s `onPress={() => onEditPress(task.id)}` unchanged; `TaskCardBody`'s `onEdit` wired to the same `onEditPress(task.id)` (see above)
- `src/components/FocusedTaskCard.tsx` — consumes `TaskCardBody` with `layout: 'stacked'` for its Title/Date/Labels/Links/Edit/Complete/Delete, rather than the directly-duplicated implementation the "Folding in PR #45" section originally specified, and rather than the separately-planned standalone pencil icon from the "Inline quick-edit" section above (superseded — see above); the `isEditing`/`TaskQuickEdit` swap and `onRefresh` prop threading from that section are otherwise unchanged, just now sitting in front of `TaskCardBody` instead of duplicated JSX; outer `TouchableOpacity`'s `onPress` gains the `if (!isEditing)` guard described above

### Test Plan addition

- `TaskCardBody`-driven rendering: confirm `TaskRow` (All view) and `FocusedTaskCard.tsx` (Focused/Today/Tomorrow) render identically-behaving Links/Complete/Delete despite the shared component, with only the intentional layout difference (actions top-right vs. bottom row) between them
- `FocusedTaskCard.tsx` shows exactly one Edit affordance (the `TaskCardBody` icon, not a separate pencil): tapping it opens inline quick-edit; tapping elsewhere on a non-editing card still opens the full `TaskFormScreen` modal; tapping elsewhere while editing does not open the modal
- `TaskRow` (All view): tapping the new `TaskCardBody` Edit icon and tapping anywhere else on the row both open the same `TaskFormScreen` modal, with no behavior regression from today

---

## Sneezy's Review — 2026-07-05

**Tier:** LIGHT — per spawn instructions, scope limited strictly to the new "## Open Questions — Resolved (user decision, 2026-07-05)" section (its three numbered items and the two subsections beneath it, "Files to Modify (further addition...)" and "Test Plan addition"); everything above that heading was explicitly out of scope for this pass, already covered by two prior Sneezy passes (2026-07-02 ×2, 2026-07-04 ×2) — same convention as the 2026-07-04 first LIGHT pass in this file. No escalation trigger found: the only files this section touches or adds (`TaskCardBody.tsx` (new, RN), `TaskRow` in `TasksScreen.tsx`, `FocusedTaskCard.tsx`) are all presentation-only RN components — no model/schema/router/API-contract file, no data-model change, single-component (mobile) OTA deployment, unchanged from the rest of this plan.

**Verdict:** Changes required

### Issues

1. **[Blocker]** The new section's placement creates a direct, nearby self-contradiction rather than a clean append. This "Open Questions — Resolved" section (lines 295–316) declares the narrow-extraction-vs-duplication question resolved (superseded by a full shared-component decision). But every other dated addition in this file so far has been appended at the growing *end* of the file — and this one was not: it was inserted *before* the already-existing "Sneezy's Review — 2026-07-04 (second pass, same day)" section and its "Grumpy's Response," which sit immediately below it (lines 319–363) despite being dated a day earlier. That older response's closing line (confirmed by reading it) reads: "Open Question #1 (narrow extraction vs. duplication) remains open; see Open Questions above." A reader following the file top-to-bottom hits "resolved" first and "remains open" ~50 lines later, about the same question, with no note reconciling the two. Fix by either moving this new section to the true end of the file (after the 2026-07-04 second-pass content, matching every other section's append order), or by editing the stale "remains open" sentence to point forward to this resolution.

2. **[Risk]** Item 2's directed fix — a full RN port of `TaskCardBody.tsx` consumed by both `TaskRow` and `FocusedTaskCard.tsx` — creates a duplicate Edit affordance on `FocusedTaskCard.tsx` that this section doesn't reconcile. Confirmed by reading `frontend/src/components/TaskCardBody.tsx` in full: `actionsEl` (lines 103–147) unconditionally renders an Edit button (lines 120–128, `onClick={(e) => { e.stopPropagation(); onEdit(); }}`) alongside Complete/Delete — gated only on `task.state === 'pending'`, not on `priorityBadge` mode or any other flag, so both the `'stacked'` (Focused/Today/Tomorrow) and `'inline'` (All) layouts get it. This plan's still-current "Inline quick-edit" section (line 104, out of scope for direct re-review here, but its output is exactly what this section's own "Files to Modify" bullet (line 311) builds on) already separately adds "an `isEditing` state and a pencil-icon button" to `FocusedTaskCard.tsx`. Line 311 says the `isEditing`/`TaskQuickEdit` swap is "unchanged, just now sitting in front of `TaskCardBody` instead of duplicated JSX" — but doesn't address that `TaskCardBody` itself already renders its own Edit button as part of what it "owns." Implemented literally, `FocusedTaskCard.tsx` ends up with two edit affordances (its own pencil icon, plus `TaskCardBody`'s built-in edit icon in the actions row). This needs an explicit call: either the RN port's actions surface omits Edit for the `'stacked'` case (making the port not a literal 1:1 match, contrary to how this section frames the decision — "mirroring web's architecture"), or `FocusedTaskCard.tsx`'s separately-planned pencil icon is dropped in favor of `TaskCardBody`'s own.

3. **[Gap]** The description of what the RN `TaskCardBody` "owns" (line 309: "Title/Date/Labels/Links/priority-indicator/Complete/Delete rendering") is incomplete relative to the real component it's ported from. Confirmed gaps: (a) Edit-button rendering is omitted from the list (see Issue 2); (b) none of the four callback props the real component actually requires (`onEdit`, `onComplete`, `onDelete`, `onTogglePriority`) are named — only "priority-badge style" is mentioned, with no statement of whether mobile needs an `onTogglePriority` equivalent at all (neither `TaskRow` nor `FocusedTaskCard.tsx` currently has any priority-toggle-from-card affordance, so presumably both mobile call sites are `priorityBadge: 'static'`-only and never wire `onTogglePriority` — plausible, but unstated); (c) the real component's `actionsEl` is gated on `task.state === 'pending'` (`TaskCardBody.tsx:103`) — hiding Edit *and* Delete, not just Complete, whenever a task isn't pending. This is plausibly inert for mobile (this plan's earlier, already-approved "Show completed" removal should mean `TaskRow`/`FocusedTaskCard` never receive a non-pending task once implemented), but this section doesn't note the dependency or confirm it — worth one explicit line either way, since currently `TaskRow`'s Delete button (`TasksScreen.tsx:143-149`) has no `isDone` gate at all, so this would be a live behavior change if the "Show completed" removal doesn't fully eliminate `'done'` tasks from `TaskRow`'s input in every code path.

4. **[Nit]** Item 1's resolution ("This was the plan's existing default; no plan changes needed") doesn't note that the original scope-decision text it confirms — "Scope decision (**unconfirmed** — asked the user, no response received, proceeding with the recommended default)" (line 101) — is now stale phrasing that reads as still-open. A one-line update or forward pointer at line 101 (e.g., "confirmed 2026-07-05, see Open Questions below") would keep the file internally consistent without requiring a reader to already know to check the bottom of the file.

### Unverified assumptions

- The new section's characterization of `TaskCardBody.tsx`'s prop signature and layout-branch behavior (layout `'inline' | 'stacked'`, `dateDisplay`, `priorityBadge`, `onTogglePriority`, `renderLabels`, and the stacked-vs-inline action-placement split) is **confirmed accurate** — read the full file (`frontend/src/components/TaskCardBody.tsx`, 178 lines). The `'stacked'` branch (lines 150–160) and `'inline'` branch (lines 163–177) match the description exactly; action-button placement is driven purely by the `layout` prop, not an unfinished parity gap, as claimed.
- Whether mobile's `TaskRow` needs to keep its own outer whole-row `TouchableOpacity` (`onPress={() => onEditPress(task.id)}`, confirmed at `TasksScreen.tsx:85-86`, wrapping the *entire* row including the future `TaskCardBody`) once `TaskCardBody`'s own internal Edit button also exists inside it — could not be resolved from the plan text. Unlike web's `TaskCard.tsx`, which has a distinct outer "navigate to detail" click deliberately separate from `TaskCardBody`'s own Edit button (confirmed: `TaskCard.tsx:57`, `onClick={() => { if (!isEditing) navigate(...); }}`, with `isEditing` state living in `TaskCard.tsx` itself), mobile's `TaskRow` has no "view detail" concept at all — tapping anywhere on the row today directly opens the edit modal. Whether that outer tap survives unchanged, gets removed in favor of the new internal Edit button, or the two are meant to coexist (tapping the row anywhere, or the dedicated icon, both open the same modal) is not stated.
- Whether the `task.state === 'pending'`-gated `actionsEl` (hiding Delete along with Edit/Complete for non-pending tasks) is actually inert for mobile post-implementation — plausible given this plan's earlier "Show completed" removal, but not independently re-verified against the (out-of-scope) `listTasks(...)` fetch call site in this pass.

### Suggestions

- Move this "Open Questions — Resolved" section to the true end of the file (after the existing 2026-07-04 second-pass review and its Grumpy response), or edit that response's closing "remains open" sentence to point forward here — resolves Issue 1 without needing new analysis.
- Add an explicit line stating whether `FocusedTaskCard.tsx` keeps its own separately-planned pencil-icon Edit button or instead relies on `TaskCardBody`'s own internal Edit button once adopted — a literal port of both duplicates the affordance.
- Add a Test Plan bullet verifying `FocusedTaskCard.tsx` shows exactly one Edit affordance (not two) after the `TaskCardBody` refactor, and that `TaskRow`'s existing whole-row tap-to-edit behavior is either unchanged or explicitly redefined once it wraps the shared component.
- State explicitly whether `onTogglePriority`/`priorityBadge: 'toggle'` has any RN equivalent planned, or confirm both mobile call sites use `priorityBadge: 'static'` with `onTogglePriority` unused — avoids an implementer wondering whether a new feature (priority toggle from the card) is silently in scope.

— *Sneezy*

---

## Grumpy's Response to Sneezy's 2026-07-05 Review

| Sneezy item | Status |
|---|---|
| Blocker 1 (section inserted before, not after, the 2026-07-04 second-pass content — self-contradicting with the "remains open" line) | Addressed — "Open Questions — Resolved" moved to the true end-of-file append position (after the 2026-07-04 second-pass Grumpy response), and that response's stale "remains open" sentence rewritten to point forward |
| Risk 2 (literal `TaskCardBody` port creates a duplicate Edit affordance on `FocusedTaskCard.tsx`) | Addressed — read web's `TaskCard.tsx`/`FocusedTaskCard.tsx` and mobile's current `TaskRow`/`FocusedTaskCard.tsx` in full to work out the correct wiring: `FocusedTaskCard.tsx` drops its separately-planned pencil icon in favor of `TaskCardBody`'s own Edit icon (wired to `onEdit={() => setIsEditing(true)}`, exactly matching web), with the existing outer whole-card tap gaining an `if (!isEditing)` guard; `TaskRow` keeps its existing whole-row tap unchanged and wires the new icon to the same callback — no duplicate, both resolved to single coherent behaviors |
| Gap 3 (`TaskCardBody`'s owns-list and callback props incomplete; `onTogglePriority`/pending-gate unaddressed) | Addressed — new subsection states the full prop surface (`onEdit`/`onComplete`/`onDelete`/`onTogglePriority`), confirms both mobile call sites use `priorityBadge: 'static'` with no toggle affordance, and flags the `pending`-state gate as carried over with an explicit build-time check tied to the "Show completed" removal |
| Nit 4 (stale "unconfirmed" wording at line 101) | Addressed — updated to "confirmed by user 2026-07-05," with a forward pointer |

All four issues addressed; this plan's outstanding open questions are now fully resolved. Implementation may proceed.
