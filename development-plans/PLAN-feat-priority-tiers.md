...
- **Doc (arch review):** `ARCHITECTURE.MD` updated (mobile Code Structure entries, test counts, the `isFormHighPriorityEligible`→`isFormPriorityEligible` rename). `DATA_MODEL_AND_API.MD` needed no changes (no backend/API files touched). Zero architectural concerns. Flagged six pre-existing stale paragraphs in `ARCHITECTURE.MD` ("High-priority tasks", "Overdue column", "Day After Tomorrow column", "Monday column", "Drag-to-reorder within a column", "Day View") that still described backend/web priority logic in terms of the legacy `is_high_priority` boolean, pre-dating PR1/PR2.
- **Follow-up doc cleanup (2026-08-08, same PR, per user request):** Re-ran Doc scoped specifically to the six flagged paragraphs, verified against current `main` source (PR1/PR2 already merged). Fixed (commit `cb5456d`): tri-state `priority` field, `isHighPriorityEligible`→`isPriorityEligible` rename, 2-zone→3-zone (High/Medium/Normal) column description, `is_high_priority`→`priority`/`TIER_RANK` drag-drop payload and sort key, backend `Task.priority == 'high'` filter. Found and fixed two additional pieces of staleness discovered during verification, unrelated to PR1/PR2: (1) the doc's old "`isHighPriorityEligible` returns `true` for overdue" claim was already wrong *before* this epic — that function has always returned `false` for `'overdue'`, confirmed via `git log -p`; (2) web (PR #73) and mobile (PR3/#74) genuinely diverge on Overdue-column sort order — web generalized to the full 3-way `TIER_RANK`, mobile deliberately kept a 2-way high/not-high split (Sneezy's Gap finding) — now documented as an intentional platform divergence rather than implied symmetry. `DATA_MODEL_AND_API.MD` cross-checked, needed no changes.
...
- **`mobile/src/utils/taskPriority.ts`'s `splitByPriority` removed, not generalized** — confirmed dead (no callers outside its own test) per Sneezy's verified finding; the 3-way split isn't needed anywhere in mobile's SectionList-based UI.
- **`taskGrouping.ts`'s `overdue` bucket:** implemented exactly as Sneezy specified — primary comparator renamed from `is_high_priority` to `priority === 'high'` (still a 2-way high/not-high split, not the 3-way tier rank), secondary tiebreak (`updated_at` descending) untouched. `upcoming` untouched entirely. Only the `else` bucket got the full `TIER_RANK` (high<medium<normal) + `sort_order` tiebreak, mirroring web's `TasksPage.tsx` `TIER_RANK` constant.
- All 3 target test files (`taskPriority.test.ts`, `taskGrouping.test.ts`, `taskFilters.test.ts`) rewritten/updated; `npx jest` — 106/106 passing (full mobile suite, not just the 3 touched files). `npx tsc --noEmit` — zero errors outside `__tests__/` (the test-file "Cannot find name describe/it/expect" errors are a pre-existing tsconfig gap, confirmed present identically on untouched files like `taskLinks.test.ts`, not introduced by this PR).
- **`FocusedTaskCard.tsx`/`FocusedView.tsx` needed zero changes** (the original "Files to Modify (PR3)" list expected trivial field-name updates there) — both delegate entirely to `TaskCardBody.tsx`, which already carries the field-name fix for both its `static` and `toggle` badge modes. Confirmed correct, not an oversight (per Dopey's code review).
- **Post-Dopey fix (2026-08-08):** `TasksScreen.tsx`'s `handleTogglePriority` now distinguishes a `422` (daily-cap-reached) response with a specific alert, matching `TaskFormScreen.tsx`'s existing pattern — closes the narrow race window between the client-side `canAddHighPriority` pre-check and the `updateTask` call (Dopey's one Should-fix item; no Must-fix items).
