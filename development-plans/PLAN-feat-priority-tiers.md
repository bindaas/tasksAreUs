...
## Status
**State:** In progress — PR1 (backend) merged; PR2 (web, #73) merged to `main`; PR3 (mobile) not started.
**Last updated:** 2026-08-08 by Grumpy
**Next step:** PR2 ([#73](https://github.com/bindaas/tasksAreUs/pull/73)) merged to `main` — full review chain complete. Dopey (code review): no blocking issues, posted as a comment with 3 nits + 2 clarifying questions (both nits fixed in a follow-up commit before merge — `ColumnPriorityCollapseContext` now imports `PriorityTier` from `api/tasks.ts` instead of redeclaring it; the tier-cycle-then-demote and drop-eligibility-demote logic extracted from `TasksPage.tsx`'s handlers into tested pure functions `resolveNextPriorityTier`/`resolveDropPriority` in `taskPriority.ts`). Sleepy (test review): added integration coverage for the priority-only PUT path (previously only proven via POST), all tests pass. Bashful: `PRODUCT_REQUIREMENTS_DOCUMENT.MD` updated, aspirational 3-tier markers removed for the web-shipped pieces. Doc: `ARCHITECTURE.MD` updated for all touched files; `DATA_MODEL_AND_API.MD` needed no changes (frontend-only PR, no new API contract). Merged via `gh pr merge --merge` (regular merge commit, matching PR1's convention); `feat-priority-tiers-web` branch left in place (not auto-deleted, matching PR1's convention). Next: start PR3 (mobile) on a fresh branch off `main` — see PR3 section above for scope (3-way tier sort/UI on `TasksScreen`/`TaskCardBody`/`TaskFormScreen`/`FocusedTaskCard`/`FocusedView`/`SettingsScreen`; `taskGrouping.ts`'s sort-key generalization only applies to the `else`/today-tomorrow-dayAfter/nodate bucket per Sneezy's Gap finding — `overdue` and `upcoming` buckets keep their existing, different tiebreaks).
**Blocked on:** n/a — awaiting go-ahead to start PR3.
