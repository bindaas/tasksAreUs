...
## Summary
- 1804 nodes · 4143 edges · 125 communities (77 shown, 48 thin omitted)
...
Cohesion: 0.09
Nodes (33): getDayViewTasks(), FocusedBoard, getFocusedViewTasks(), BoardGroupedTasks(), DayView(), EmptyState(), FolderIcon(), StarIcon() (+25 more)
...
Cohesion: 0.16
Nodes (17): BoardCompletions, CompletionRecord, CompletionsReport, getCompletions(), GetCompletionsOptions, ArchiveBoardGroups(), CompletionCard(), formatDateTime() (+9 more)
...
### Community 111 - "frontend/src/context/BoardContext.tsx"
Cohesion: 0.10
Nodes (29): all_boards Additive Query Param Design, Board-Grouped Collapsible Rendering (mirrors Today view), PLAN: Rename Reports → Archive, add date-range presets and board filtering, Board Color Everywhere (TaskCard/BoardTabs accents), Plan: Custom board order + board-color styling + alphabetical tags, Native input[type=color] Auto-Save Picker (routes through setColorBoard), Plan: feat-focused-view-web (PR 2 of 3), PLAN-feat-multi-board-backend (PR #33, external, merged) (+21 more)
...
### Community 115 - "TaskDetailPage.tsx"
Cohesion: 0.13
Nodes (19): PLAN-feat-inline-tag-add, Inline Tag Creation from TaskForm (avoid Settings trip), apiFetch(), createLabel(), deleteLabel(), LabelCategory, listLabels(), updateLabel() (+11 more)
...
Cohesion: 0.08
Nodes (37): UI Naming Change: Type → Tags, PLAN: Frontend & Mobile Type Cleanup (PR 4, follow-up), Narrow Label.category Union / Remove Dead recurrence_group_id Type, completeTask(), CompleteTaskBody, CompleteTaskResponse, createTask(), CreateTaskBody (+29 more)
...
## Knowledge Gaps
- **477 isolated node(s):** `Status`, `Branch`, `Background`, `Current behavior (all three items root-caused by code trace)`, `Scope decisions (flagging for explicit sign-off, not proceeding silently)` (+472 more)
...
- **Why does `Development Plan: feat-focused-view (backend, PR 1 of 3)` connect `Task` to `schemas.py`, `mobile/src/api/focusedView.ts`, `frontend/src/context/BoardContext.tsx`?**
  _High betweenness centrality (0.134) - this node is a cross-community bridge._
- **Why does `Plan: feat-focused-view-mobile (PR 3 of 3)` connect `mobile/src/api/focusedView.ts` to `Task`, `TasksScreen.tsx`, `apiFetch`, `frontend/src/context/BoardContext.tsx`?**
