...
Then, add these to the project `.claude/settings.local.json` under `permissions.allow`.
...
8. **Merge into `.claude/settings.local.json`** in the current project (not `~/.claude/settings.json`, not `.claude/settings.json`). Create the file if it doesn't exist. Preserve existing keys and existing entries in `permissions.allow`; de-duplicate against what's already there; don't remove anything; don't reorder unrelated fields.
